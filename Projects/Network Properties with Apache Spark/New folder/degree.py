...
def simple(g):
	...
	flippedEdges = g.edges.selectExpr("src as dst","dst as src").select("src","dst")
	...
	combinedEdges = g.edges.unionAll(flippedEdges).distinct()
	combinedEdges = combinedEdges.where("src != dst")
	
	return GraphFrame(g.vertices, combinedEdges)
...
def degreedist(g):
	...
	print(g.inDegrees.head(3))
	
	degreeCounts = g.inDegrees.groupBy("inDegree").count()
	return degreeCounts
...
def readFile(filename, large, sqlContext=sqlContext):
	...
	#edgeslist = lines.map(lambda x: x.split(delim)).flatMap(lambda x: [(x[0],x[1]),(x[1],x[0])])
	edgeslist = lines.map(lambda x: x.split(delim)).map(lambda x: (x[0],x[1])).distinct()
	edges = sqlContext.createDataFrame(edgeslist,['src','dst'])
	...
	# graphframe vertices
	verticeslist = lines.flatMap(lambda x: x.split(delim)).map(lambda x: (x,)).distinct()
	vertices = sqlContext.createDataFrame(verticeslist,['id'])

...
def records_to_graph():
    ...
    nodelist = []
    nodedict = {}
    id_count=0 #Give id to all nodes

    #Create nodes for users
    for u in ratings.keys():
        node = Node(id_count,u,type="user")
    ...
    for m in movies.keys():
        node = Node(id_count,m,type="movie")
        nodelist.append(node)
        nodedict[m] = node
        id_count += 1

    #Create nodes for ratings for each movie
    #and add edges between movie-ratings and movie
    for m in movies.keys():
        for r in range(1,6):
            r_name = m + "_" + str(r) 
            node = Node(id_count,r_name,type="rating")
            nodelist.append(node)
            nodedict[r_name] = node
            #create edges both ways
            nodedict[r_name].neighbors.append(nodedict[m])
            nodedict[m].neighbors.append(nodedict[r_name])

            id_count += 1

    #Create nodes for directors
    for d in directors:
        node = Node(id_count,d,type="director")
        nodelist.append(node)
        nodedict[d] = node
        id_count += 1
    ...
    for a in actors:
        node = Node(id_count,a,type="actor")
        nodelist.append(node)
        nodedict[a] = node
        id_count += 1
    ...
    for g in genres:
        node = Node(id_count,g,type="genre")
        nodelist.append(node)
        nodedict[g] = node
        id_count += 1
    ...
    for u in ratings.keys():
        for m in ratings[u].keys():
            rating = ratings[u][m]
            rating_key = m + "_" + str(rating)
            #create edges
            nodedict[rating_key].neighbors.append(nodedict[u])
            nodedict[u].neighbors.append(nodedict[rating_key])

    
    # Add edges between movies and actors, directors and genres
    for m in movies.keys():
        #create edge between movie and director
        d = movies[m].director
        if d:
            nodedict[d].neighbors.append(nodedict[m])
            nodedict[m].neighbors.append(nodedict[d])
        #create edge between movie and actors
        for a in movies[m].actors:
            nodedict[a].neighbors.append(nodedict[m])
            nodedict[m].neighbors.append(nodedict[a])
        #create edge between movie and genres
        for g in movies[m].genres:
            nodedict[g].neighbors.append(nodedict[m])
